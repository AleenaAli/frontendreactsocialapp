    ...
    switch (action.type) {
      ...
      case "bodyRules":
        if (!action.value.trim()) {
          ...
          draft.body.message = "You must provide a content for body"
        }
        return
      case "notFound":
        draft.notFound = true
  ...
  return (
    <Page title="Edit Post">
      ...
      <form className="mt-3" onSubmit={submitHandler}>
        ...
        <div>{state.title.hasErrors && <div className="alert alert-danger small liveValidateMessage">{state.title.message}</div>}</div>
        <div className="form-group">
        ...
        <div>{state.body.hasErrors && <div className="alert alert-danger small liveValidateMessage">{state.body.message}</div>}</div>
